...
if (!function_exists('relative_time_v3')) {
    function relative_time_v3($time, $decimals = 1, $output = NULL, $returnArray = false, $returnFormattedNumber = true, $returnCompactString = false)
    {
        ...
        if (time() > $time) {
            ...
            if ($number == 1) {
                $timeString = !$returnCompactString ? $timeString : $timeString . '.';
            } else {
                $timeString = !$returnCompactString ? $timeString . 's' : $timeString . 's.';
            }
        }
...
if (!function_exists("checkLogin")) {
    ...
    {
        global $_COOKIE;
        if (isset($_COOKIE['mygds_session'])) {
            global $db;
...
if (!class_exists('steam_webapi')) {
    class steam_webapi
    {
        ...
        {
            $APIresult = curl('https://api.steampowered.com/ISteamUser/ResolveVanityURL/v0001/?key=' . $this->steamAPIKey . '&vanityurl=' . $vanityURL, NULL, NULL, NULL, NULL, 5, 5);
        ...
        function GetFriendList($steamID, $relationshipFilter = 'friend')
        {
            ...
            $APIresult = curl('https://api.steampowered.com/ISteamUser/GetFriendList/v0001/?key=' . $this->steamAPIKey . '&steamid=' . $steamID . '&relationship=' . $relationshipFilter);
        ...
        function GetPlayerSummariesV2($steamID)
        {
             ...
             */
            try {
                $APIresult = curl('https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v2/?key=' . $this->steamAPIKey . '&steamids=' . $steamID);
        ...
        function GetPublishedFileDetails($wid)
        {
            try {
                ...
                $APIresult = curl('https://api.steampowered.com/ISteamRemoteStorage/GetPublishedFileDetails/v1/', $postFields);
...
if (!function_exists('makeLineChart')) {
    ...
    function makeLineChart($dataArray, $div_container = 'container', $chartTitle, $chartSubtitle = NULL, $series1Options = array('title' => 'Games', 'min' => 0, 'yAxis' => array('reversed' => false)), $seriesNOptions = array('Series2' => array('title' => 'Test', 'yAxis' => 1, 'opposite' => true)), $seriesTooltipFormat = array('pointFormat' => 'Value: {point.y:.2f} mm'))
    {
        require_once(__DIR__ . "/bootstrap/highcharts/Highchart.php");
        require_once(__DIR__ . "/bootstrap/highcharts/HighchartJsExpr.php");
        require_once(__DIR__ . "/bootstrap/highcharts/HighchartOption.php");
        ...
        $chart->yAxis[0]->min = isset($series1Options['min'])
            ? $series1Options['min']
            : 0;
        $chart->yAxis[0]->reversed = isset($series1Options['yAxis']['reversed'])
            ? $series1Options['yAxis']['reversed']
            : false;
        /*$chart->tooltip->formatter = new HighchartJsExpr(
            "function() {
...
if (!function_exists('makeColumnChart')) {
    //$seriesOptions -- Add series name as key, and all options in array attached to it will be applied to series in chart
    function makeColumnChart($chartData = array(0 => array('title' => 'Ballot Series', 'data' => array('Nov' => 123, 'Dec' => 123))), $div_container = 'container', $chartTitle, $chartSubtitle = NULL, $chartYaxisTitle = 'Votes', $seriesTooltipFormat = array('pointFormat' => 'Value: {point.y:.2f} mm'))
    {
        require_once(__DIR__ . "/bootstrap/highcharts/Highchart.php");
        require_once(__DIR__ . "/bootstrap/highcharts/HighchartJsExpr.php");
        require_once(__DIR__ . "/bootstrap/highcharts/HighchartOption.php");
        ...
        $chart->yAxis[0]->title->text = $chartYaxisTitle
            ? $chartYaxisTitle
            : 'Games';
        $chart->yAxis[0]->min = isset($chartData[0]['min'])
            ? $chartData[0]['min']
            : 0;
        ...
        if (!empty($chartData)) {
            foreach ($chartData as $key => $value) {
                ...
                $chart->series[$key]->name = !empty($value['title'])
                    ? $value['title']
                    : 'Unknown Series';

                if (!empty($value['data'])) {
                    foreach ($value['data'] as $key2 => $value2) {
                        $chart->xAxis->categories[$i] = $key2;
                        $chart->series[$key]->data[$i] = $value2;
                        $i++;
                    }
                }
            }
        ...
        if ($seriesTooltipFormat == array('pointFormat' => 'Value: {point.y:.2f} mm') || empty($seriesTooltipFormat)) {
            $chart->tooltip->formatter = new HighchartJsExpr(
                ...
                $(order).each(function(i,j){
                    if(points[j].y > 0){
                        temp += "<b>" + points[j].series.name + ": " + Highcharts.numberFormat(points[j].y, 0, ".", ",") + "</b><br/>";
                    }
                });
                return temp;
            }'
...
if (!class_exists('irc_message_v2')) {
    class irc_message_v2
    {
        private $webhook_url = '';
        private $db = null;
        ...
        public function __construct($webhook_gds_site, $db = null)
        {
            $this->set_webhook($webhook_gds_site);

            if (!empty($db)) $this->db = $db;
            if (!empty($memcached)) $this->memcached = $memcached;
        }
        ...
        public function queue_message($message, $owner)
        {
            if (empty($this->db)) throw new Exception("No DB instance defined!");
            //if (empty($this->memcached)) throw new Exception("No memcached instance defined!");
            if (empty($this->webhook_url)) throw new Exception("No webhook defined!");

            $sql = $this->db->q(
                "INSERT INTO `gds_irc_queue`
                      (`message_content`, `message_webhook`, `message_owner`, `message_sent`)
                      VALUES (?, ?, ?, '0')",
                'sss',
                array(
                    $message,
